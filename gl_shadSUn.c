...
out vec4 fColor;\
void main() {\
  gl_Position = mfv * vec4(vertex, 1.0);\
  fColor = aColor;\
}";
...
// Initialize the shader and link them into a program 
  ...
  if(ulMvp == -1) printf(" ************ ERROR glGetUniformLocation mfv\n");
...
//----------------------------------------------------------------
  // ortho:
  // set view-matrix
  glUniformMatrix4fv (shSUn.ulMvp, 1, GL_FALSE, (float*)mfv);
...
//----------------------------------------------------------------
  ...
  for(i1=0; i1<DL_nr; ++i1) {
    ...
    glDrawArrays (GL_TRIANGLES, DL__[i1].bOff / 12, vtxNr);
