...
Displaylist permanent objects; manage all attributes
...
// DL_add__      create new DL-record
  ...
  DL__[DL_nr].objID = objID;     // store object-ID of obj
  DL__[DL_nr].glOff = iOff;      // store bufferOffset of obj
  ...
  for(i1=0; i1<DL_nr; ++i1) {
    if(DL__[i1].typ < Typ_GL_Sur) {
     ...
     printf("%3d typ=%2d dbi=%2d iOff=%3d disp=%d hili=%d dim=%d %s\n",i1,
            DL__[i1].typ, DL__[i1].dbi, DL__[i1].glOff,
