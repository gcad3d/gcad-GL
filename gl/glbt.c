...
//   retCode    >= 0 - selected button;   -1 = no button selected
  ...
  float posX_ndc = (float) GR_Xndc_Xpx (posXpx);
    // printf(" GLBT_sele__-posX_ndc=%f\n",posX_ndc);


  // loop tru buttons
  for(ib=0; ib<I2D_iNr; ++ib) {
    if(posX_ndc < I2D_posTab[ib]) continue;
    if(posX_ndc < I2D_endTab[ib]) {
...
//================================================================
  ...
  glUniform3fv (shSY2.ulAtt, 1, COL_YELLOW);

  // set low-left (-1,-1 in NDC)
  ptStart[0] = -1.f; 
  ptStart[1] = -1.f;
...
//     if(I2D_stat[ib] != '0') continue;
    ...
    // dispa tag
    glDrawArrays (GL_TRIANGLE_FAN, symTab[iSym_squ].iOff / 12, 4);
  ...
  // set low-left (-1,-1 in NDC) + 1 millimeter up right
  ptStart[0] = -1.f + (1.f * GR_sizX_mm_ndc);  // 0.95f;
  ptStart[1] = -1.f + (1.f * GR_sizY_mm_ndc);  // -0.95f;

  // set model-view-matrix; goto startpoint NDC bottom-left
  M44FC_transl__ ((float*)mxt, ptStart[0], ptStart[1], 0.f);
    // M44FC_dump ((float*)mxt, "GLBT_render-mSY2f-1");
...
//     }
  ...
  //----------------------------------------------------------------
    DEB_dump_vc (&GR_eyeZ, " ex-GLBT_render-GR_eyeZ");
    M44D_dump__ (GR_matd_mdl[0], " ex-GLBT_render-GR_matd_mdl[0]");
